...
for i in range(len(left_image_path)):
    ...
    for row in range(half_window, height - half_window, 4):
        for col in range(half_window, width - half_window, 4):
            ld = left_disparity[row, col]
            while ld/8 == float('inf') or ld/8 == 0 or ld/8 + half_window > col or \
                    right_disparity[row, int(col - ld/8)]/8 == float('inf') or \
                    right_disparity[row, int(col - ld/8)]/8 == 0 or \
                    np.fabs(right_disparity[row, int(col - ld/8)] - ld) > 1:
